...
=======
		...
		btnThem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				NhanVien nv = getForm();
				if (nv.getMaNV().isEmpty() || nv.getHoTen().isEmpty()) {
					JOptionPane.showMessageDialog(NhanVienFrame.this, "Vui lòng nhập đầy đủ thông tin!", "Thông báo",
							JOptionPane.WARNING_MESSAGE);
					return;
				}
				boolean ok = controller.addNhanVien(nv);
				if (ok) {
					JOptionPane.showMessageDialog(NhanVienFrame.this, "Thêm thành công!", "Thông báo",
							JOptionPane.INFORMATION_MESSAGE);
					loadTable();
					clearForm();
				} else {
					JOptionPane.showMessageDialog(NhanVienFrame.this, "Thêm thất bại!", "Thông báo",
		...
		btnSua.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				NhanVien nv = getForm();
				if (nv.getMaNV().isEmpty()) {
					JOptionPane.showMessageDialog(NhanVienFrame.this, "Vui lòng chọn nhân viên để sửa!", "Thông báo",
							JOptionPane.WARNING_MESSAGE);
					return;
				}
				boolean ok = controller.updateNhanVien(nv);
				if (ok) {
					JOptionPane.showMessageDialog(NhanVienFrame.this, "Cập nhật thành công!", "Thông báo",
							JOptionPane.INFORMATION_MESSAGE);
					loadTable();
				} else {
					JOptionPane.showMessageDialog(NhanVienFrame.this, "Cập nhật thất bại!", "Thông báo",
							JOptionPane.ERROR_MESSAGE);
				}
			}
		});
...
>>>>>>> 7138b57 (E)
		...
		btnXoa.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String maNV = txtMaNV.getText().trim();
				if (maNV.equals(AuthUtil.getUser().getMaNV())) {
					JOptionPane.showMessageDialog(NhanVienFrame.this, "Bạn không thể xóa tài khoản đang đăng nhập!",
							"Thông báo", JOptionPane.WARNING_MESSAGE);
					return;
				}
				if (maNV.isEmpty()) {
					JOptionPane.showMessageDialog(NhanVienFrame.this, "Vui lòng chọn nhân viên để xóa!", "Thông báo",
							JOptionPane.WARNING_MESSAGE);
					return;
				}
				int confirm = JOptionPane.showConfirmDialog(NhanVienFrame.this, "Xóa nhân viên này?", "Xác nhận",
						JOptionPane.YES_NO_OPTION);
				if (confirm == JOptionPane.YES_OPTION) {
					boolean ok = controller.deleteNhanVien(maNV);
					if (ok) {
						JOptionPane.showMessageDialog(NhanVienFrame.this, "Xóa thành công!", "Thông báo",
								JOptionPane.INFORMATION_MESSAGE);
						loadTable();
						clearForm();
					} else {
						JOptionPane.showMessageDialog(NhanVienFrame.this, "Xóa thất bại!", "Thông báo",
								JOptionPane.ERROR_MESSAGE);
					}
				}
			}
	...
	private void loadTable() {
		...
		for (NhanVien nv : list) {
			tableModel.addRow(new Object[] { nv.getMaNV(), nv.getHoTen(), nv.getTenDangNhap(), nv.getMatKhau(),
					nv.getEmail(), nv.getGioiTinh(), nv.getVaiTro() });
		}
	}
